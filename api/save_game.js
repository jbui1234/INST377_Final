...
export default async function handler(req, res) {
  ...
  try {
    ...
    const searchRes = await fetch(`https://www.cheapshark.com/api/1.0/games?title=${encodeURIComponent(title)}`);
    ...
    if (!deal) return res.status(404).json({ error: 'No deals found' });

    const storeRes = await fetch('https://www.cheapshark.com/api/1.0/stores');
    const storeList = await storeRes.json();
    const storeName = storeList.find(s => s.storeID === deal.storeID)?.storeName || `Store ${deal.storeID}`;
